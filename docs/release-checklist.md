...
- Smoke the Windows Event Log disclosure before its first decision: choosing **Disable** must make no lookup, and **Continue** must start only after the displayed frame. Verify Activity presents any result only as an `Unverified Windows Event Log record`, retains it after **Clear**, and shows no raw XML or executable path.
