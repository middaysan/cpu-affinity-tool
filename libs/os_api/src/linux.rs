...
impl OS {
    ...
    pub fn get_program_path_for_uri(uri_scheme: &str) -> Result<PathBuf, String> {
        ...
        let output = Command::new("xdg-mime")
            .args(&["query", "default", &format!("x-scheme-handler/{}", uri_scheme)])
            .output()
        ...
        if desktop_file.is_empty() {
            return Err(format!("No default application found for URI scheme: {}", uri_scheme));
        }
        ...
        for dir in &desktop_dirs {
            ...
            if desktop_path.exists() {
                ...
                return Self::parse_desktop_file(&desktop_path)
                    .map(|(path, _)| path);
            }
        }
